...
RSpec.describe PasswordsController, type: :controller do
  describe '#update' do
    context 'with registered user' do
      ...
      let(:user) { create :user, :operator, password: current_password, password_confirmation: current_password }

      before { login_user(user) }

      it 'изменение пароля с правильным текущим паролем' do
        put 'update', params: { change_password_form: { current_password: current_password,
