...
RSpec.describe Amlapp::DocumentKindFieldDefinitionsController, type: :controller do
  ...
  it '#create' do
    post :create, params: attributes_for(:aml_document_kind_field_definition, document_kind_id: aml_document_kind.id)
    expect(response).to be_redirect
  end

  it '#update' do
    put :update, params: { id: aml_document_kind_field_definition.id, key: 'newkey', title: 'newtitle', document_kind_id: aml_document_kind.id }
    expect(response).to be_redirect
  end
end
